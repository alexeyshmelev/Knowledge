...
double scalar(vec<double> v0, vec<double> v1, int n, int m, int* sendcounts, int* displs, MPI_Comm comm, int world_size, int myrank){

    vec<double> v0_scattered(m);
    vec<double> v1_scattered(m);
    double sum[world_size];

    MPI_Scatterv(v0.raw_ptr(), sendcounts, displs, MPI_DOUBLE, v0_scattered.raw_ptr(), m, MPI_DOUBLE, 0, comm);
    MPI_Scatterv(v1.raw_ptr(), sendcounts, displs, MPI_DOUBLE, v1_scattered.raw_ptr(), m, MPI_DOUBLE, 0, comm);

    double rq[1];
    rq[0] = 0.;
    for (int j=0; j<m; j++)
    {
        rq[0] += v1_scattered(j) * v0_scattered(j);
    }

    MPI_Allgather(rq, 1, MPI_DOUBLE, sum, 1, MPI_DOUBLE, comm);

    double summary = 0.;
    for (int i = 0; i < world_size; i++) summary += sum[i];

    return summary;

}

double max_difference(vec<double> v0, vec<double> v1, int n, int m, int* sendcounts, int* displs, MPI_Comm comm, int world_size, int myrank, double rq){

    vec<double> v0_scattered(m);
    vec<double> v1_scattered(m);
    double diffs[world_size];
    ...
    for (int j=0; j<m; j++)
    {
        d = v1_scattered(j) / rq - v0_scattered(j);
        if (std::abs(d) > diff[0])
        {
            diff[0] = d;
        }
    ...
    MPI_Allgather(diff, 1, MPI_DOUBLE, diffs, 1, MPI_DOUBLE, comm);

    double max_diff = diffs[0];
    for (int i = 0; i < world_size; i++) if (diffs[i] > max_diff) max_diff = diffs[i];
...
double eigenvalue(matrix<double> A, MPI_Comm comm, int* sendcounts, int* displs)
{
    ...
    while (std::abs(diff) > 1e-15)
    {
        ...
        rq = scalar(v0, v1, n, m, recvcounts_new, displs_new, comm, world_size, myrank);

        diff = max_difference(v0, v1, n, m, recvcounts_new, displs_new, comm, world_size, myrank, rq);
